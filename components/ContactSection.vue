<template>
    <div class="max-w-2xl mx-auto">
        <h2 class="text-center text-4xl font-medium">Neem contact op</h2>

        <p class="mt-6">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus lobortis dolor a nisl venenatis lobortis. Fusce lacinia tempus mauris, a faucibus elit tristique quis.</p>

        <div class="flex gap-12 justify-center mt-6">
            <a href="mailto:mail@meespostma.nl">
                <Button title="E-mail" class="px-12" />
            </a>

            <a href="https://nl.linkedin.com/in/mees-postma-a911b1196" target="_blank">
                <Button title="LinkedIn" class="px-12" />
            </a>
        </div>
    </div>
</template>